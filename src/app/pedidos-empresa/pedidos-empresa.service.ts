...
export class PedidosEmpresaService {
  ...
  getPedidosEmpresa(): Observable<PedidoEmpresa[]> {
    return (collectionData(this.pedidosEmpresaRef, { idField: 'id' }) as Observable<PedidoEmpresa[]>).pipe(runInZone(this.zone));
  }

  getPedidoEmpresa(id: string): Observable<PedidoEmpresa> {
    return (docData(doc(this.firestore, `pedidos_empresa/${id}`), { idField: 'id' }) as Observable<PedidoEmpresa>).pipe(runInZone(this.zone));
  }
